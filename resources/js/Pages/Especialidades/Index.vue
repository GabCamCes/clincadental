...
<template>
    ...
    <div class="max-w-3xl mx-auto mt-10">
        ...
        <table v-else-if="especialidades && especialidades.data && especialidades.data.length > 0" class="w-full bg-white rounded shadow mb-6">
            <thead>
            ...
            <tbody>
                <tr v-for="esp in especialidades.data" :key="esp.id" class="border-b hover:bg-gray-50">
                    ...
                    <td class="py-2 px-3 text-center flex gap-2 justify-center">
                        <Link :href="route('especialidades.edit', esp.id)" class="text-blue-600 underline">Editar</Link>
                        <form :action="route('especialidades.destroy', esp.id)" method="post" @submit.prevent="$inertia.delete(route('especialidades.destroy', esp.id))" class="inline">
                            <button type="submit" class="text-red-600 underline" onclick="return confirm('¿Eliminar?')">Eliminar</button>
                        </form>
                    </td>
                </tr>
            </tbody>
