...
<template>
    ...
    <div class="max-w-5xl mx-auto mt-10">
        ...
        <table v-else-if="pacientes && pacientes.data && pacientes.data.length > 0" class="w-full bg-white dark:bg-slate-800 rounded shadow mb-6">
            <thead>
            ...
            <tbody>
                <tr v-for="p in pacientes.data" :key="p.id" class="border-b hover:bg-gray-50">
                    ...
                    <td class="py-2 px-3 text-center flex gap-2 justify-center">
                        <Link :href="route('pacientes.edit', p.id)" class="btn btn-edit">Editar</Link>
                        <form :action="route('pacientes.destroy', p.id)" method="post" @submit.prevent="$inertia.delete(route('pacientes.destroy', p.id))" class="inline">
                            <button type="submit" class="btn btn-danger" onclick="return confirm('¿Eliminar paciente?')">Eliminar</button>
                        </form>
                    </td>
                </tr>
            </tbody>
