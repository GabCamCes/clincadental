...
const form = useForm({ nombre: props.sala.nombre, descripcion: props.sala.descripcion })
function submit() { form.put(route('salas.update', props.sala.id)) }
</script>
...
<template>
    ...
    <div class="max-w-md mx-auto mt-10 bg-white shadow rounded px-8 py-6">
        <h2 class="text-xl font-bold mb-6">Editar Sala</h2>
        <form @submit.prevent="submit">
            ...
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('salas.index')" class="text-gray-700 underline ml-4">Cancelar</Link>
            </div>
        </form>
    </div>
    <footer
