...
function submit() { form.put(route('productos.update', props.producto.id)) }
</script>
    ...
    <div class="max-w-md mx-auto mt-10 bg-white shadow rounded px-8 py-6">
        ...
        <form @submit.prevent="submit">
            ...
            <div class="mb-4">
                ...
                <input v-model="form.precio" type="number" step="0.01" min="0" class="w-full border px-3 py-2 rounded" />
                <div v-if="form.errors.precio" class="text-red-500 text-sm">{{ form.errors.precio }}</div>
            </div>
            <div class="mb-4">
                <label class="block mb-1">Stock</label>
                <input v-model="form.stock" type="number" min="0" class="w-full border px-3 py-2 rounded" />
            ...
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('productos.index')" class="text-gray-700 underline ml-4">Cancelar</Link>
            </div>
        </form>
    </div>
    <footer
